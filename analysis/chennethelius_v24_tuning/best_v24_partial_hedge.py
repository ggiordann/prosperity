"""Round 3 v24 - v23 with partial VEV delta hedging.

The VEV options are calls on VFE. Each long call has positive delta toward
VFE, so large option inventory creates implicit VFE exposure. This strategy
keeps the v16/v23 structure and offsets only part of that option delta:

  option_delta = sum(position_i * BS_delta_i)
  hedged_vfe_target = kalman_vfe_target - HEDGE_GAIN * option_delta

The Kalman mean-reversion target stays primary. The hedge is only a bias term,
not a full replacement for the VFE signal.
"""
...
def override_float(overrides, key, default):
    try:
...
def override_int(overrides, key, default):
    try:
...
INFORMED_SIZE_VFE = override_int(RUNTIME_OVERRIDES, "informed_size_vfe", 11)
INFORMED_GAIN_S = 10
