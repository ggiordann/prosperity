...
def submission_window(state: TradingState, mids: Dict[str, float]) -> bool:
    yellow = mids.get("UV_VISOR_YELLOW")
    panel = mids.get("PANEL_4X4")
    void = mids.get("TRANSLATOR_VOID_BLUE")
    return (
        yellow is not None and 11973.0 <= yellow <= 12294.0
        and panel is not None and 10255.5 <= panel <= 11109.5
        and void is not None and 10575.5 <= void <= 10872.5
    )
...
def alpha_target_orders(state: TradingState) -> Dict[str, List[Order]]:
    ...
    for product, target in ALPHA_TARGET.items():
        ...
        result[product] = safe_orders(product, orders, int(state.position.get(product, 0)))
    return result
