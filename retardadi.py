...
class Trader:
    """Round 3 Velvet voucher trader from a repository-fitted Black-Scholes smile."""
    ...
    # Fitted from round3 price CSVs in prosperity_rust_backtester/datasets/round3
    # on strikes 5000-5500 after excluding the largest residual outlier (5400).
    SMILE_A = 0.01305220
    SMILE_B = 0.01281602
    SMILE_C = 0.27669756
    ...
    def option_size(self, strike: int) -> int:
        return self.STRIKE_SIZE.get(strike, self.DEFAULT_SIZE)
    ...
    def trade_voucher(
        ...
        max_clip = self.option_size(strike)
        orders: List[Order] = []

        for ask_price, ask_volume in sorted(order_depth.sell_orders.items()):
            if buy_capacity <= 0:
                break
            if ask_price + self.MIN_EDGE <= fair_value:
        ...
        for bid_price, bid_volume in sorted(order_depth.buy_orders.items(), reverse=True):
            if sell_capacity <= 0:
                break
            if bid_price - self.MIN_EDGE >= fair_value:
