...
class Trader:
    ...
    Design:
    - ASH_COATED_OSMIUM: anchored around a stable fair value near 10_000.
      We market-make *inside* the spread with an inventory-skewed reservation price,
      and we also sweep clearly stale visible quotes.

    - INTARIAN_PEPPER_ROOT: persistent positive drift.
      The main fix versus the previous version is to reach max long inventory much
      earlier by sweeping favorable ask levels and then improving the best bid by
      one tick for additional passive fills.

    The logic is low-parameter and product-regime based rather than day-specific.
    """
    ...
    def _trade_pepper(
        ...
        # 1) Aggressively accumulate by sweeping favorable visible asks.
        for ask in sorted(order_depth.sell_orders):
            if buy_cap <= 0:
                break
            ask_qty = -order_depth.sell_orders[ask]
            if ask <= trend_fair + self.PEPPER_TAKE_EDGE:
                qty = min(buy_cap, ask_qty)
                if qty > 0:
                    orders.append(Order(product, ask, qty))
                    buy_cap -= qty

        # 2) Improve the best bid by one tick for additional passive fills.
        if buy_cap > 0:
    ...
    def _trade_ash(self, state: TradingState, order_depth: OrderDepth) -> List[Order]:
        ...
        # 1) Sweep clearly stale visible quotes.
        for ask in sorted(order_depth.sell_orders):
            if buy_cap <= 0:
                break
            ask_qty = -order_depth.sell_orders[ask]
        ...
        # 2) Market-make inside the spread, skewed by inventory.
        ...
        if sell_cap > 0 and (best_bid is not None or best_ask is not None):
            ...
            if ask_price is not None and (best_bid is None or ask_price > best_bid):
