...
OB_ACTIVE = {"ROBOT_LAUNDRY", "ROBOT_VACUUMING", "ROBOT_MOPPING", "ROBOT_DISHES", "SLEEP_POD_NYLON", "TRANSLATOR_VOID_BLUE", "MICROCHIP_OVAL"}
...
def top(depth: OrderDepth):
    return max(depth.buy_orders), min(depth.sell_orders)


def book_shift(product: str, depth: OrderDepth) -> float:
    if product not in OB_ACTIVE:
        return 0.0
    bid, ask = top(depth)
    bv = max(0, int(depth.buy_orders[bid]))
    av = max(0, -int(depth.sell_orders[ask]))
    den = bv + av
    if den <= 0:
        return 0.0
    micro = (ask * bv + bid * av) / den - (bid + ask) / 2.0
    return BOOK_WEIGHT * micro


def dynamic_size(mid_history: List[float], spread: int) -> int:
    if len(mid_history) < 8:
        return SIZE
    changes = [abs(mid_history[i] - mid_history[i - 1]) for i in range(len(mid_history) - 6, len(mid_history))]
    vol = sum(changes) / max(1, len(changes))
    if vol > max(2.0, 1.25 * spread):
        return 4
    if vol > max(1.0, 0.75 * spread):
        return 7
    return SIZE


def trade_take(product: str, depth: OrderDepth, pos: int, fair: float, take: float, size: int) -> List[Order]:
    best_bid, best_ask = top(depth)
    spread = best_ask - best_bid
    buy_cap = max(0, LIMIT - pos)
    sell_cap = max(0, LIMIT + pos)
    min_take = take
    orders: List[Order] = []
    if buy_cap > 0 and best_ask <= fair - min_take:
        qty = min(buy_cap, size, -int(depth.sell_orders[best_ask]))
        if qty > 0:
            orders.append(Order(product, best_ask, qty))
    if sell_cap > 0 and best_bid >= fair + min_take:
...
def trade_book(product: str, depth: OrderDepth, pos: int, fair: float, take: float, edge: float, improve: int, plain: bool, size: int) -> List[Order]:
    best_bid, best_ask = top(depth)
    spread = best_ask - best_bid
    buy_cap = max(0, LIMIT - pos)
    sell_cap = max(0, LIMIT + pos)
    ...
    if product in WALK and buy_cap > 0:
        for price in sorted(depth.sell_orders):
            if price > fair - min_take or buy_cap <= 0:
                break
            qty = min(buy_cap, size, -int(depth.sell_orders[price]))
            if qty > 0:
                orders.append(Order(product, price, qty))
                buy_cap -= qty
    elif buy_cap > 0 and best_ask <= fair - min_take:
        qty = min(buy_cap, size, -int(depth.sell_orders[best_ask]))
        if qty > 0:
            orders.append(Order(product, best_ask, qty))
            buy_cap -= qty
    if product in WALK and sell_cap > 0:
        for price in sorted(depth.buy_orders, reverse=True):
            if price < fair + min_take or sell_cap <= 0:
                break
            qty = min(sell_cap, size, int(depth.buy_orders[price]))
            if qty > 0:
                orders.append(Order(product, price, -qty))
                sell_cap -= qty
    elif sell_cap > 0 and best_bid >= fair + min_take:
        qty = min(sell_cap, size, int(depth.buy_orders[best_bid]))
        if qty > 0:
            orders.append(Order(product, best_bid, -qty))
            sell_cap -= qty
    ...
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            if product in ROUND5 and depth.buy_orders and depth.sell_orders:
                ...
                arr = mid_hist.get(product, [])
                arr.append(mids[product])
                mid_hist[product] = arr[-12:]
        ...
        for group in ACTIVE:
            for product in GROUPS[group]:
                ...
                if VARIANT == "pure_mm":
                    fair = mids[product] - SKEW_WEIGHT * spread * pos
                    orders = trade_book(product, depth, pos, fair, BIG, max(1.0, spread * 0.18), max(1, min(4, spread // 3)), False, size)
                ...
                if VARIANT in ("hybrid", "fair_mm", "crossing_only", "basket"):
                    for leader, lag, weight in SIG.get(product, ()):
                        arr = hist.get(leader, [])
                        if len(arr) > lag:
                            fair += weight * (arr[-1] - arr[-1 - lag])

                if VARIANT in ("imbalance_only", "hybrid"):
                    if VARIANT == "imbalance_only":
                        fair = mids[product]
                        take = BIG
                    fair += book_shift(product, depth)

                if VARIANT == "basket":
                    peers = GROUPS[PRODUCT_GROUP[product]]
                    vals = [mids[p] - (FAIR.get(p, mids[p]) + SHIFT.get(p, 0.0)) for p in peers if p in mids]
                    if vals:
                        fair += 0.15 * sum(vals) / len(vals)

                fair -= SKEW_WEIGHT * spread * pos

                if VARIANT == "crossing_only" or product in TAKE_ONLY:
                    orders = trade_take(product, depth, pos, fair, take, size)
                else:
                    edge = EDGE[product]
                    if VARIANT == "imbalance_only":
                        edge = max(1.0, spread * 0.25)
                    orders = trade_book(product, depth, pos, fair, take, edge, IMP.get(product, 0), product in PLAIN_QUOTE, size)
                result[product] = safe_orders(product, orders, pos)

        return result, 0, json.dumps({"c": circle_hist, "h": hist, "m": mid_hist}, separators=(",", ":"))
