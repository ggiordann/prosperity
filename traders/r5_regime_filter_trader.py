...
PRODUCT_GROUP = {p: group for group, products in GROUPS.items() for p in products}
STABLE_PRODUCTS = {
    "GALAXY_SOUNDS_BLACK_HOLES", "GALAXY_SOUNDS_DARK_MATTER", "GALAXY_SOUNDS_SOLAR_FLAMES", "GALAXY_SOUNDS_SOLAR_WINDS",
    "MICROCHIP_OVAL", "MICROCHIP_RECTANGLE", "MICROCHIP_SQUARE", "MICROCHIP_TRIANGLE",
    "OXYGEN_SHAKE_CHOCOLATE", "OXYGEN_SHAKE_EVENING_BREATH", "OXYGEN_SHAKE_MINT",
    ...
    "TRANSLATOR_ECLIPSE_CHARCOAL", "TRANSLATOR_GRAPHITE_MIST", "TRANSLATOR_SPACE_GRAY", "TRANSLATOR_VOID_BLUE",
    "UV_VISOR_AMBER", "UV_VISOR_MAGENTA", "UV_VISOR_ORANGE", "UV_VISOR_YELLOW",
}
...
def trade_book(product: str, depth: OrderDepth, pos: int, fair: float, take: float, edge: float, improve: int, plain: bool, size: int, passive: bool) -> List[Order]:
    best_bid = max(depth.buy_orders)
    best_ask = min(depth.sell_orders)
    buy_cap = max(0, LIMIT - pos)
    sell_cap = max(0, LIMIT + pos)
...
def push_hist(store, product: str, value: float, limit: int) -> None:
    arr = store.get(product, [])
    arr.append(round(float(value), 4))
    if len(arr) > limit:
        arr = arr[-limit:]
    store[product] = arr
...
def std(values) -> float:
    if len(values) < 2:
        return 0.0
    m = avg(values)
    return (sum((x - m) * (x - m) for x in values) / (len(values) - 1)) ** 0.5
...
def corr(a, b) -> float:
    ...
    am = avg(a)
    bm = avg(b)
    av = sum((x - am) * (x - am) for x in a)
    bv = sum((x - bm) * (x - bm) for x in b)
    if av <= 0 or bv <= 0:
        return 0.0
    return sum((x - am) * (y - bm) for x, y in zip(a, b)) / ((av * bv) ** 0.5)


def regime_scale(product: str, mid: float, fair: float, spread: float, depth_total: int, mids: Dict[str, float], mids_hist) -> float:
    edge = fair - mid
    abs_edge = abs(edge)
    scale = 1.0
    ...
    if len(mids_hist) >= 25:
        short_trend = mids_hist[-1] - mids_hist[-6]
        trend_len = min(40, len(mids_hist) - 1)
        trend = mids_hist[-1] - mids_hist[-1 - trend_len]
        ...
        for group in ACTIVE:
            for product in GROUPS[group]:
                ...
                if not depth or not depth.buy_orders or not depth.sell_orders:
                    result[product] = []
                    continue
                if product not in STABLE_PRODUCTS:
                    result[product] = []
                    continue
                if product in MID:
                ...
                if scale <= 0:
                    result[product] = []
                    continue
                size = max(1, min(LIMIT, int(SIZE * scale)))
                passive = scale >= 0.75
                if take < BIG / 2:
                ...
                if product in TAKE_ONLY:
                    ...
                    orders = trade_book(product, depth, pos, fair, take, edge, IMP.get(product, 0), product in PLAIN_QUOTE, size, passive)
